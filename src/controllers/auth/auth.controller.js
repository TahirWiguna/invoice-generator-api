...
exports.login = async (req, res) => {
    ...
    try {
        ...
        if (error) {
            return response(res, responseType.VALIDATION_ERROR, "Form Validation Error", error.details);
        }
        ...
        if (user.active === false) return response(res, responseType.FORBIDDEN, "User is inactive");

        if (user.banned_until > new Date()) return response(res, responseType.FORBIDDEN, "User has been banned");

        const validatePassword = await bcrypt.compare(password, user.password);
        if (!validatePassword) return response(res, responseType.NOT_FOUND, "username/password wrong");
